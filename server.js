...
app.post('/upload', upload.single('image'), (req, res) => {
    ...
    pythonProcess.on('close', (code) => {
        ...
        try {
            ...
            res.status(500).json({ error: 'Invalid JSON from detection.py', details: stdout });
        }
    });
});

app.get('/', (req, res) => {
    res.send('<h2>Node Image Upload Server is running.</h2><p>POST an image to /upload with form-data key "image".</p>');
});
